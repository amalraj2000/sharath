      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-12">
          ...
          <motion.div 
            ...
            className="bg-white p-8 rounded-xl shadow-sm border border-concrete-200 hover:shadow-md transition-shadow"
          >
            <div className="flex items-center gap-3 mb-8">
              <div className="bg-industrial-navy p-2 rounded-lg">
                <PenTool className="w-6 h-6 text-white" />
              </div>
              <h3 className="text-2xl font-semibold text-industrial-navy">Technical & Software</h3>
            </div>
              ...
              {skills.technical.map((skill, index) => (
                <div key={index}>
                  <div className="flex justify-between mb-1">
                    <span className="font-medium text-concrete-900">{skill.name}</span>
                    <span className="text-concrete-800 text-sm">{skill.level}%</span>
                  </div>
                  <div className="w-full bg-concrete-100 rounded-full h-2.5 overflow-hidden">
                    <motion.div 
                      ...
                      className="bg-safety-amber h-2.5 rounded-full" 
                    ></motion.div>
                  </div>
                </div>
              ))}
            ...
            >
              {skills.domain.map((skill, index) => (
                ...
                >
                  ...
                  <span className="font-medium text-concrete-900">{skill}</span>
                </motion.div>
              ))}
            </motion.div>
          </motion.div>
